...
CONFIDENCE_THRESHOLD = 0.65   # Landmarks below this are ignored
...
class PullUpCounter:
    ...
    def get_stats(self):
        """Return live stats for the Flask bridge."""
        return {
            "reps": self.counter,
            "stage": (self.stage or "DOWN").upper(),
            "confidence": round(self._confidence * 100, 1),
            "form_warnings": list(self._form_warnings),
        }

    def _check_confidence(self, landmarks, indices):
        """Return mean visibility for given landmark indices."""
        vals = [landmarks[i].visibility for i in indices]
        return float(np.mean(vals)) if vals else 0.0

    def _check_form(self, landmarks, frame_shape):
        """Return list of form warning strings."""
        h, w = frame_shape[:2]
        warnings = []
        MP = self.mp_pose.PoseLandmark

        def xy(lm): return (lm.x * w, lm.y * h)

        # ── Arm symmetry: wrists should be at roughly same height
        lw = landmarks[MP.LEFT_WRIST.value]
        rw = landmarks[MP.RIGHT_WRIST.value]
        if lw.visibility > CONFIDENCE_THRESHOLD and rw.visibility > CONFIDENCE_THRESHOLD:
            diff = abs(lw.y - rw.y) * h
            if diff > 40:
                warnings.append("⚠ Uneven grip — align both hands")

        # ── Head position: nose should not be tilted > ~20% off centre
        nose = landmarks[MP.NOSE.value]
        ls   = landmarks[MP.LEFT_SHOULDER.value]
        rs   = landmarks[MP.RIGHT_SHOULDER.value]
        if (nose.visibility > CONFIDENCE_THRESHOLD
                and ls.visibility > CONFIDENCE_THRESHOLD
                and rs.visibility > CONFIDENCE_THRESHOLD):
            shoulder_cx = (ls.x + rs.x) / 2
            tilt = abs(nose.x - shoulder_cx)
            if tilt > 0.12:
                warnings.append("⚠ Head tilting — look straight ahead")

        # ── Back alignment: shoulders should be level (not rotation)
        if ls.visibility > CONFIDENCE_THRESHOLD and rs.visibility > CONFIDENCE_THRESHOLD:
            shoulder_diff = abs(ls.y - rs.y) * h
            if shoulder_diff > 35:
                warnings.append("⚠ Shoulders uneven — keep back straight")
    ...
    def process_frame(self, frame):
        ...
        try:
            if results.pose_landmarks:
                ...
                # ── Confidence gate: only count if key joints are reliable ──
                key_indices = [
                    self.mp_pose.PoseLandmark.LEFT_WRIST.value,
                    self.mp_pose.PoseLandmark.RIGHT_WRIST.value,
                    self.mp_pose.PoseLandmark.NOSE.value,
                ]
                self._confidence = self._check_confidence(landmarks, key_indices)
                ...
                if self._confidence >= CONFIDENCE_THRESHOLD:
                    ...
                    # ── Form check ──
                    self._form_warnings = self._check_form(landmarks, frame.shape)
                    for i, warn in enumerate(self._form_warnings):
                        cv2.putText(image, warn,
                                   (10, 160 + i * 22),
                                   cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 165, 255), 1, cv2.LINE_AA)
                else:
                    # Low confidence — show warning on frame
                    current_status = self.stage.upper() if self.stage else "DOWN"
                    cv2.putText(image, f"Low confidence ({int(self._confidence*100)}%) - adjust camera",
                               (10, 160),
