...
def build_network(image, layers, variables):
    ...
    def _separable_conv(inputs, stride, block_id, dilations):
        ...
        w = tf.nn.depthwise_conv2d(
            inputs, _depthwise_weights(dw_layer), stride, 'SAME', dilations=dilations, data_format='NHWC')
...
def convert(model_id, model_dir, check=False):
    ...
    with cg.as_default():
        ...
        with tf.compat.v1.Session() as sess:
            ...
            # Use modern TensorFlow 2.x approach to save the model
            tf.io.write_graph(cg, model_dir, "model-%s.pbtxt" % chkpoint, as_text=True)
            
            # Convert to frozen graph using tf.compat.v1.graph_util
            ...
            save_path = os.path.join(model_dir, "model-%s.pb" % chkpoint)
